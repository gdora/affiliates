...
class WPAM_List_Affiliates_Table extends WPAM_List_Table {
    ...
    function get_columns() {
        $columns = array(
            ...
            'partnerName' => __('Partner Name', 'affiliates-manager'), // company name
            'dateCreated' => __('Date Joined', 'affiliates-manager'),
            //'broker_accounts' => __('Broker Accounts', 'affiliates-manager'),
    ...
    function prepare_items($ignore_pagination = false) {
        ...
        $query = "select
                $aff_table_name.*,
                ( select concat(firstName, ' ', lastName) from $aff_table_name prt where $aff_table_name.partner_id = prt.affiliateId ) partnerName,
                (
