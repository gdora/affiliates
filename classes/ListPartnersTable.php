...
class WPAM_List_Affiliates_Table extends WPAM_List_Table {
    ...
    function get_columns() {
        $columns = array(
            ...
            'bountyAmount' => __('bountyAmount', 'affiliates-manager'),
            //'companyName' => __('Partners', 'affiliates-manager'), // company name
            'dateCreated' => __('Date Joined', 'affiliates-manager'),
            //'websiteUrl' => __('Website', 'affiliates-manager'),
            'viewDetail' => __('View Detail', 'affiliates-manager'),
        );
        return $columns;
    }
    ...
    function prepare_items($ignore_pagination = false) {
        ...
        $query = "select
                ...
                +
				(
				select round(COALESCE(SUM(IF(status <> 'failed', amount,0)),0) * $aff_table_name.bountyAmount / 100, 2) 
					from " . WPAM_TRANSACTIONS_TBL . " tr 
					where tr.affiliateId in (select partner.affiliateId from " . WPAM_AFFILIATES_TBL . " partner where partner.partner_id=". WPAM_AFFILIATES_TBL . ".affiliateId)
					and tr.status != 'failed' 
