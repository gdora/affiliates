...
$isPartner = WPAM_Plugin::isPartner($model->affiliateId);
?>
    ...
    <table class="pure-table wpam-contact-info">
        ...
        <tbody>
            ...
            foreach ($this->viewData['affiliateFields'] as $field) {
                $value = '';
                if (!is_admin() && ($field->fieldType == "partner" || $field->fieldType == "partner_id")) {
					continue;
				}
                //prefer request data
                    ...
                    <td>
                        ...
                        switch ($field->fieldType) {
                            ...
                            case 'partner':
								if (is_admin()) {
									?>
									<select name="_<?php echo $field->databaseField; ?>">
									<option value="0" <?php if ($value == 0) echo "selected"; ?>><?php _e('Affiliate', 'affiliates-manager') ?></option>
									<option value="1" <?php if ($value == 1) echo "selected"; ?>><?php _e('Partner', 'affiliates-manager') ?></option>
									</select>
									<?php
								} 
                                break;
                            case 'partner_id':
								if (is_admin()) {
									global $wpdb;
									$data = $wpdb->get_results("select * from " . WPAM_AFFILIATES_TBL . " where partner=1", ARRAY_A);
									print "<select name='_" . $field->databaseField . "'>";
									print "<option value='0'></option>";
									foreach ($data as $partner) {
										if  ($partner['affiliateId'] == $value) {
											$selected = "selected";
										} else {
											$selected = "";
										}
										 print "<option value='{$partner['affiliateId']}' $selected>{$partner['firstName']} {$partner['lastName']}</option>";
									}
									print "</select>";
...
<?php } ?>
        ...
        ?>
                    ...
                    <tr id="rowPaypalEmail" <?php echo $pp_email_field_style; ?>>
                        ...
                        <td>
                            <input id="txtPaypalEmail" type="text" name="txtPaypalEmail" size="30" value="<?php echo $this->viewData['paypal 	'] ?>"/>
                        </td>
                    </tr>
    <?php endif; ?>
                   <?php if (is_admin() && WPAM_Plugin::isPartner($model->affiliateId)){ ?>					  
                    <tr>
                        <td>
        ...
        <?php 
			...
			print '<input type="hidden" id="ddBountyType" name="ddBountyType" value="percent">';
			print "<input type='hidden' id='txtBountyAmount' name='txtBountyAmount' size='5' value='10'>";
