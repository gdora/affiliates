...
<div id="tabs">
    <ul>
		...
		<?php
		if (WPAM_Plugin::isPartner($model->affiliateId)) {
			?>
...
<?php } ?>
		...
		$brokerAccounts = json_decode($this->viewData['affiliate']->broker_accounts);
				...
				<table class="widefat">
			<thead>
				<tr><th colspan="3"><?php _e("Broker Accounts", 'affiliates-manager'); ?></th></tr>
				</thead>
				<tbody>
				<tr>
		...
		<table class="widefat">
			<thead>
				<tr>
					<th colspan="2"><?php _e( 'Users', 'affiliates-manager' ) ?></th>
				</tr>
			</thead>
			<tbody>
				<tr>
				...
				$query = "select concat(af.firstName, ' ', af.lastName) name, COALESCE(SUM(IF(tr.status <> 'failed', amount,0)),0) standing 
					from " . WPAM_TRANSACTIONS_TBL . " tr, " . WPAM_AFFILIATES_TBL . " af
					where 
					af.partner_id = " . $model->affiliateId . 
				...
				foreach ($users as $user) {
					print "<tr><td>" . $user->name . "</td><td>" . $user->standing . "</td></tr>";
	...
	<div id="fragment-4">
		...
		<table class="widefat">
			...
			<tr class="creative-<?php echo $creative->status?>">
				...
				<td><input type="text" size="50" value='<?php echo htmlentities( get_site_url() . "/index.php/affiliate-home/affiliate-register/" . $model->affiliateId, ENT_QUOTES ) ?>' /></td>
			</tr>
			<?php //} ?>
			</tbody>
		</table>
