...
jQuery(document).ready(function($){
	//jQuery("#_partner").parent().parent().remove();
	//jQuery("#_partner_id").parent().parent().remove();

	//jQuery("#_partner").partent("tr").remove();
...
global $wp;
if ($wp->query_vars['page']) {
	$partner_id = $wp->query_vars['page'];
	
	
	global $wpdb;
	$data = $wpdb->get_results("select * from " . WPAM_AFFILIATES_TBL . " where affiliateId=$partner_id", ARRAY_A);
	if ($data[0]) {
		$parnter_name = $data[0]['firstName'] . " " . $data[0]['lastName'];
		print "<h2>Partner: $parnter_name</h2>";
	}
} else {
	$partner_id = 0;
}
...
<form action="" method="post" id="mainForm" class="pure-form">
		...
		<table class="pure-table">
			<?php foreach ($this->viewData['affiliateFields'] as $field) { 
				...
				if ($field->fieldType != 'partner' and $field->fieldType != 'partner_id') {
			...
			<tr>
                ...
                <div class="wpam-registration-form">
                    ...
                    <input type="hidden" name="_partner_id" value="<?php echo $partner_id ?>" />
                    <input type="hidden" name="_partner" value="0" />
