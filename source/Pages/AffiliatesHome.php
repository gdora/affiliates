...
class WPAM_Pages_AffiliatesHome extends WPAM_Pages_PublicPage
{
			...
			if ($db->getAffiliateRepository()->isUserAffiliate($currentUser->ID))
			{
				...
				if ($affiliate->isApproved() || $affiliate->isActive())
				{
					...
					$response->viewData['navigation'] = array(
						...
						//Sarray( __( 'Sales', 'affiliates-manager' ), $this->getLink(array('sub' => 'sales'))),
	...
	protected function doOverviewHome($request, $affiliate)
	{
		...
		if ($_POST['action'] == "delete") {
			$c = 0;
			$remove = -1;
			$broker_accounts = json_decode($affiliate->broker_accounts);
			foreach ($broker_accounts as $account) {
				if ($_POST['broker_name'] == $account->name && $_POST['broker_id'] == $account->id && $_POST['broker_type'] == $account->type) {
					$remove = $c;
				}
				$c++;
			}
			if ($remove >= 0) {
				unset($broker_accounts[$remove]);
				$affiliate->broker_accounts = json_encode($broker_accounts);
				$db->getAffiliateRepository()->update($affiliate);
			}
		}
		
		// add broker account
		if ($_POST['action'] == "add_broker") {
			if ($_POST['new_broker_name'] != "" && $_POST['new_broker_id'] != "" && $_POST['new_broker_type'] != "") {
				$broker_accounts = json_decode($affiliate->broker_accounts);
				$newAccount = array('name' => $_POST['new_broker_name'], 'id' => $_POST['new_broker_id'], 'type' => $_POST['new_broker_type']);
				if (!$broker_accounts) {
					$broker_accounts = array();
				}
				array_push($broker_accounts, $newAccount);
				$affiliate->broker_accounts = json_encode($broker_accounts);
				$db->getAffiliateRepository()->update($affiliate);
			} else {
				print '<span style="color: red;">All fields must be filled</span>';
			}
		...
		if ($_POST['action'] == "requestpayment") {
			$subject = translate('Payment request', 'affiliates-manager');
			$message = translate('An affiliate is requesting payment', 'affiliates-manager');
			$message .= "\n";
			$message .= translate('Name', 'affiliates-manager');
			$message .= ": " . $affiliate->firstName . " " . $affiliate->Lastname;
			$to = get_bloginfo('admin_email');
			//echo "wp_mail( $to, $subject, $message);";
			wp_mail($to, $subject, $message);
			print '<span style="color: green;">' . translate('Payment request sent to admin', 'affiliates-manager') . '</span>';
		}
		
		//print_r($affiliate);
		$accountSummary = $db->getTransactionRepository()->getAccountSummary($affiliate->affiliateId, $affiliate->bountyAmount);
